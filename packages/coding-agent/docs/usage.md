...
In fullscreen mode, click anywhere in a compact tool or thinking block to expand or collapse it. A collapsed tool keeps its one-line summary row, and expanding it reveals the full output or diff. Dragging across output still selects text, modified clicks do not toggle, and external links retain their normal behavior. The first click of a double-click also toggles the block, so drag to select a word inside it. Regular mode leaves mouse input to the terminal, so use the existing keyboard expansion shortcuts there.
